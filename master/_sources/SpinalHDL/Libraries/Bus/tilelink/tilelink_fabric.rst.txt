...
You can note that they all are Handles. Handle is a way in SpinalHDL to share a value between fibers. If a fiber read a Handle while this one has no value yet, it will block the execution of that fiber until another fiber provide a value to the Handle.
...
.. code-block:: scala
    ...
    class GpioFiber extends Area {
      ...
      val fiber = Fiber build new Area {
        ...
        // Starting by defining a 32 bits TriStateArray (Array meaning that each pin has its
        // own writeEnable bit.
